...
| Branching | Trunk-based on `main` with short-lived branches; production cuts from `release/X.Y.x` using the estate's existing release tool. A security patch does not wait for the sprint boundary: it is cherry-picked onto the live `release/X.Y.x` and promoted through the tool's hotfix path under AD-25, skipping the train and no gate. Merge to `main` deploys develop only. Branch protection requires one approval, code owners, linear history and passing checks |
