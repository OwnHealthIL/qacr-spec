...
- **Rule:** every transmitted payload carries a digest over a byte range the OpenAPI document
  ...
  **The phone number is never a scope, a query key or a lookup path** — it is an input to AD-6's
  ...
  results — is Product's, `decisions/D-01.md`; household visibility would be a new rule with an
...
- **Rule:** one narrow control-plane namespace — the `FR-PLT-005` supported operating-system and
  ...
  patch is cherry-picked onto the live `release/X.Y.x` and promoted through the release tool's hotfix
  path without waiting for the sprint boundary: it skips the release train and **never** an AD-24
  blocking gate, and its `FR-LCM-017` release testing is scoped to the change plus a regression set
...
| Branching | Trunk-based on `master` with short-lived branches; production cuts from `release/X.Y.x` using the estate's existing release tool. A security patch does not wait for the sprint boundary: it is cherry-picked onto the live `release/X.Y.x` and promoted through the tool's hotfix path under AD-25, skipping the train and no gate. Merge to `master` deploys develop only. Branch protection requires one approval, code owners, linear history and passing checks |
...
| Orchestration | GKE, Helm, helmfile; KEDA for queue-driven scaling; Jenkins for build and promotion | [OBSERVED] `helm-charts`, `Jenkinsfile` |
