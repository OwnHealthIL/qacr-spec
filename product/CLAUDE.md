...
| **E04 is triaged `Unchanged` on the engine, and the review changes the flow** | The apparent contradiction — F04.1 marked `Unchanged` with a `src: pm` note reading *"there are no new controls or mechanics"*, against sixteen departures — dissolves once the comments are read: *"the framework is the same, the chat flow structure will be a bit different"*, *"the infra is the same, the content and flow structure is different"*, and *"The structure of the chat flow in QACR will be built from scratch"*. **The triage is about the engine; the departures are about the flow it runs.** Different objects, both statements true. Settle explicitly what SPEC-04 is recreating before writing a line — this is the tripwire question, and the answer may be that the status is right and the unit was never stated. |
| **Three of E04's departures delegate to another document** | The timers to their own spec — which is SPEC-05, and *"QACR will have two other timers"* is the same two-stage shape section 10 already records; the demo controls to *"their own spec"*, which nothing yet plans; and the dipping step out of existence, since *"there's no dipping in QACR"*. **A departure that names another document is a scope boundary, not a behaviour to write.** The demo-control one has no owner: Guy also wants a navigation control to jump to a section and fast-forward past timers and audio, and that is new scope. |
...
| **E04's one `wrong` mark is the research task** | `F04.1~happy~the-visible-transcript`: not all videos are replayable, some carry a replay button and some do not, and the user should be able to scroll up to re-read. A `wrong` mark says the export's statement is untrue of the product, so it is research before it is anything else — E03's two `wrong` marks became its entire research phase. |
| **Two of E04's comments are open items, not departures** | *"regarding the different behaviors when a parameter is missing — an open question, i don't understand the behavior"* and *"regarding doctor notification — i need more context on this"*. Neither is a decision, and neither may be closed by picking the more likely reading. A third, the timeout that never arrives, states an intent Guy is confident about while saying he does not understand the current behaviour — that one is a departure with a research note attached. |
| **E04's export was read against Rev 1.20 and epic map Rev 1.14** | Three revisions behind, and unlike E03's this file declares it in `checked_against` with a `why_this_matters` note that every coverage verdict is relative to it — 24 `covered`, 58 `partly`, 4 `uncovered` at behaviour level, plus 14 candidate new requirements and 4 marked needed. **Re-map before believing a gap.** Six of E03's ten proposals were requirements that already existed. |
| **E04's export covers three of SPEC-04's seven features** | F04.1, F04.3, F04.4 — nine requirements. **F04.2, F04.5, F04.6 and F04.7 have no developers' input at all**, and those are the two `Changed` on Guy's own note, the derived `Changed` and the `New` — the features SPEC-04 was going to settle the behaviour-section shape on. The export cannot settle it. Guy's brief has to. |
