...
| **Four of E04's departures are parity-direction decisions, and they are ours** | Where the two platforms differ, Guy has chosen the target: the permission bubble and photo upload *"should work like iOS"*, the idle window after a timeout likewise, and the `wrong` item wants scroll-up-to-re-read *"like android"*. **These are not the parity question that was closed.** That decision was that *verifying* parity is QA's at the end; choosing which behaviour QACR adopts where they disagree is product content and belongs in the departures table. Do not drop them by misapplying the closure. |
| **E04's `wrong` mark needed no research at all** | `F04.1~happy~the-visible-transcript` was expected to become a research phase, as E03's two `wrong` marks did. It did not. Its first half — *"not all videos are replayable"* — **is already the requirement**: FR-FLW-003 reads *where the demonstration for a step is designated replayable*, and its note records that this was confirmed at review. The export's statement is wrong against the FR document as well as the product, and Guy's mark agrees with the requirement. Its second half, scroll-up-to-re-read *"like android"*, is a parity direction and became a departure. **Check a `wrong` mark against the current requirement text before opening a research task** — the answer may already be written down. E04's research phase is empty. |
