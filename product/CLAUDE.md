...
├── reviews/       the team's behaviour-review exports, with Guy's marks. TRACKED
...
`reviews/` — the development team's behaviour-review exports per epic, with Guy's
`pm_mark` on every line, and any disposition written against one. **Tracked**, unlike
`local/`: the export is their work and the marks are the answer to it. Read
`reviews/README.md` before using one — three things are true of every export so far, and
each of them has already cost a revision somewhere.
...
5. **Render to PDF and read the pages as images.** Text extraction cannot see a column
   ...
   `Q-3 / 6`. **`generator/render-pages.py` runs it here now** — `python3 generator/render-pages.py .`
   converts both documents headlessly and writes page PNGs to a scratch directory, and you
   read them. It needs `brew install --cask libreoffice` and `brew install poppler`, both
   one-off. Microsoft Word renders these files most faithfully of all and was tried first;
   it is rejected because driving it through AppleScript hangs on any dialog it decides to
   show, and a verification step that can hang is not one anybody will run.
...
- **Distinguish his words from yours.** In `spec-status.js`, `src: "pm"` is authoritative
  and `src: "derived"` needs confirming. Five **Changed** features still carry derived notes
  developers will read as scope: F06.1, F06.6, F08.4, F12.2, F14.1. F01.9, F04.5 and the
  derived **New** on F04.7 were among them until Guy answered each — see the tripwire in
  section 5, which now cuts both ways. **Asking is cheap and it has never once been wasted:**
  F01.9 was wrong and came back Unchanged, F04.5 and F04.7 were right and came back with a
  better note than the derivation had. Ask before writing behaviour, not after.
...
| **E04's `wrong` mark needed no research, and its second half was retracted** | `F04.1~happy~the-visible-transcript` was expected to become a research phase, as E03's two `wrong` marks did. It did not. Its first half — *"not all videos are replayable"* — **is already the requirement**: FR-FLW-003 reads *where the demonstration for a step is designated replayable*, and its note records that this was confirmed at review. Its second half asked for scroll-up-to-re-read *"like android"*, became a departure, and **Guy then withdrew it at the Rev 0.2 review** — *"wrong. i understood it incorrectly"* — leaving the plain iOS behaviour: while the flow plays, the view is carried to each new bubble. **Two lessons. Check a `wrong` mark against the current requirement text before opening a research task.** And **a mark on an export is a first reading, not a decision** — it is made against a line of someone else's prose, and the review of the spec that quotes it back is where it gets tested. Write the departure so that retracting it costs one row. |
| **Two of E04's comments are open items, not departures** | *"regarding the different behaviors when a parameter is missing — an open question, i don't understand the behavior"* and *"regarding doctor notification — i need more context on this"*. SPEC-04 U2 and U3. Neither is a decision, and neither may be closed by picking the more likely reading. A third, the timeout that never arrives, states an intent Guy is confident about while saying he does not understand the current behaviour — that is **D8, a departure with a research note attached**, and it does not wait on the research. |
| **E04's re-map against Rev 1.23 was a no-op, and that is the finding** | The export declares `checked_against: Rev 1.20 · Epic Map Rev 1.14`. Re-mapped: **of the seventeen requirements SPEC-04 owes, not one changed text between Rev 1.20 and Rev 1.23.** The three requirements added since — FR-KIT-008, FR-KIT-009, FR-KIT-010 — are all SPEC-03's and close nothing here, and E04's feature-to-requirement mapping is identical at epic map Rev 1.14 and Rev 1.17. Only FR-KIT-007 moved in the neighbourhood, and only in wording. **So every `partly` and `uncovered` verdict stands** — the opposite of E03, where six of ten proposals already existed. Do the re-map anyway; the point is that the answer is not predictable, and `product/FR-01/requirements.json` at Rev 1.20 is what makes it a two-minute diff rather than a reading exercise. |
| **Seven of E04's candidate requirements rest on behaviour Guy marked `correct`** | CN-01 to CN-04, CN-06, CN-08 and CN-11 have **every** evidence line marked `correct` — meaning the current behaviour is what QACR should do — while each exists to say the product carries no requirement for it. Sharpest is the used-kit one: *"the warning is shown once; after it is dismissed every later attempt in the window starts with no warning"* is marked `correct`, and the candidate exists to say that defeats FR-KIT-007. Under the review's own rule — anything not referenced is correct — they should be dropped. **They are recorded in SPEC-04 §6.2 rather than dropped, because for a Class B submission an unstated capability is a regulatory gap rather than a preference.** Guy's decision, not the spec writer's. Nine of the fourteen candidates are amendments to existing requirements, not new identifiers; propose them as such. |
| **E04's export covers three of SPEC-04's seven features** | F04.1, F04.3, F04.4 — nine requirements. F04.2, F04.5, F04.6 and F04.7 had no developers' input. Guy answered three of the four at the draft: **F04.5 Changed stands and is re-sourced to `pm`**, **F04.7 New stands and the spec describes the practice framing only** — the scanning interaction is E06's and is not restated — and F04.6's departure is D9. **F04.2 is still open: nothing states what the *"small changes to the video UX"* are, and no behaviour can be written for it.** That is SPEC-04 U1 and it is the one thing blocking the feature. |
...
`local/Reference/` — the Minuteful copy set and screenshots. **The behaviour reviews are
not here**, they are in `reviews/` and tracked; they were briefly under `local/` and that
was wrong, because an export from the development team and the marks on it are exactly the
thing they should be able to see.
