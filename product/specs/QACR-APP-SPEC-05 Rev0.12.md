...
| Revision | 0.12, draft for review — Rev 0.10 review and the mockup's timing mechanism applied; not yet re-reviewed |
...
**The timing mechanism from Guy's mockup is applied at this revision**, on top of the Rev 0.10
review. It confirmed the shape section 3 already had — the clock anchored to the action rather
than to the display, the card yielding before zero, the instruction playing after it — and it
supplied the mechanism underneath, which the document did not have. The interval by which the
card yields is **derived** from the length of the closing content rather than chosen. The flow
**waits for zero itself** rather than advancing on a delay. Reaching zero is a completion the
user witnesses rather than a disappearance. And the control that confirms the action appears
only once the instruction for it has finished.

**One thing the mockup and a ready document disagree about**, and it is not this spec's to
settle: the practice scan. See section 8.
...
**S05.40** **Optional content is elastic; the end-of-stage sequence is not.** The closing
sequence — transition line, zero, the completion the user witnesses, the instruction, the
control — is anchored to the clock and always happens on time. Everything optional gives way
to it.

**S05.50** That yielding happens in two ways, and both are needed. **Before an optional block
begins**, the flow checks whether enough of the stage remains for it; if not, the block does
not start at all. **Once the moment for the final action arrives**, whatever optional content
the user has not reached is skipped. The first keeps a block from starting only to be cut off
mid-way; the second catches a user who lingered. Neither ever delays the anchored sequence.

Whether the completion window turns out long enough to let a user finish the optional content
*and* still act is a question for the timing-flex study; if it does, the second rule softens
from a skip to a choice, and that is the one part of section 3 the study can still move.
...
affect neither the conduct nor the validity of the test. **It is paced across the time
available rather than at a fixed interval, and repeats where the wait outlasts the content**,
so a longer wait does not end in a card showing nothing.
...
the fixed content that has to play before zero — the transition line and anything with it — so
that the content finishes as the countdown reaches zero rather than before or after it. Each
stage has its own interval because each has its own closing content. Change the content and
the interval changes with it.

**S05.46** **The flow then waits for zero itself.** It does not advance on a fixed delay after
the closing content ends. If the content ran short the flow holds; if it ran long the
countdown is already at zero and the flow proceeds. The clock is the authority, not the
script.
...
**S05.49** The two stages are distinguishable at a glance while running, so a user can tell
which of the two they are in without reading. How is a design decision; that they are is not.
...
| S05.12 | **The practice scan is described two ways and they cannot both hold.** SPEC-04 S04.01 and FR-IMG-022, both settled at FR Rev 1.24, make it a step of the flow the user passes through in order to continue. The mockup puts it inside an optional block that is dropped entirely when too little of the stage remains. Mandatory content cannot be droppable. Either the practice scan is optional and FR-IMG-022 needs amending back, or it is mandatory and the mockup's optional block covers only the environment check and the tutorial. **This one changes a ready spec and a live requirement, so it is not a spec-level fix.** | Guy |
