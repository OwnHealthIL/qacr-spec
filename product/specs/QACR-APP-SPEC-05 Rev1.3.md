...
**Seven features, all new.** Authority is per feature — see below.
...
| Milestones | 1, except invalidation on timing grounds and telling the user why at 2, the sample-freshness window and the background end-of-stage signal at 3, the expiry notifications at 4, and the SMS alternative at 5 |
| Domains | iOS, Android |
| Traces to | QACR-APP-FR-01 Rev 1.27 · QACR-APP-EPIC-01 Rev 1.21 |
...
|---|---|
| F05.1 Timer framework and elapsed-time derivation | **new** — this document, section 3 |
| F05.2 Sample-freshness window | **new** — this document, section 3.7. The mechanism exists in Minuteful Kidney and the limit differs; the behaviour is written here |
| F05.3 Incubation gate and release-button window | **new** — this document, section 3.5 |
| F05.4 Colour-evolution gate and scanning window | **new** — this document, sections 3.4 and 3.5 |
| F05.5 Timing display and expiry notification | **new** — this document, sections 3.2, 3.4 and 3.8 |
| F05.6 Timing invalidation messaging | **new** — this document, section 3.6 |
| F04.8 Waiting-time card | **new** — this document, section 3.3 |

**Every feature here is new, and that is unusual** — SPEC-01 to SPEC-04 are mostly recreations,
where Minuteful Kidney is authoritative and the document records only departures. **These
features are not.** Minuteful Kidney has one wait of seventy-five seconds; QACR has two waits
of minutes, a card, a staged transition with its own audio timing, a practice scan, two
completion windows enforced silently, and two routes out of an invalidation. Most of it did not
exist to be recreated, and the parts that did are different in timing, in flow structure and in
content.

**So nothing here is settled by reading Minuteful Kidney**, and no behaviour in section 3 is to
be inferred from it. Where section 7 calls a mechanism *unchanged*, that says the mechanism
already exists and need not be invented — never that the behaviour lives anywhere but here.
...
Where section 7 calls a mechanism unchanged, that was checked against the current product **on
both platforms** rather than assumed — its content set, the waiting-time UX study and the
shipped applications. **How the current product achieves any of it is not recorded here and is
not this document's business**, and whether existing code can serve a requirement or has to be
written again is the development team's judgement from the requirement. Either way the
behaviour to build is the one written here.
...
| Length | 5 minutes, provisional | 4 minutes, provisional |
...
*Traces: FR-TIM-001. **Every timing value in this document is provisional.** The two durations
above, the completion windows and the scan-practice limit are what milestone 1 runs on so that
a demonstration and a rehearsal have numbers; the timing-flex study sets the real ones before
milestone 3, and FR-TIM-001 and Q-05 hold them until it does. Nothing here is under document
control and nothing here should be quoted as settled.*
...
**S05.03** Backgrounding, screen lock, a phone call, an app kill or a device restart have no
effect on the **clock**: it is wall-clock time and it keeps running. They do stop the **flow** —
nothing is delivered while the application is away. On return, elapsed time is recomputed. The
steps behave as any other chat step does; **the clock sits above them and runs regardless.** The
user resumes where they stopped and continues from there, and where too little of the stage is
left, whole sections ahead of them are dropped per S05.49. **They always meet the stage's
protected step**: it is never dropped and never skipped past, so however long they were away,
the instruction for the action is something they see. Where the window closed while they were
away, S05.04 applies instead.
...
**S05.39** The **durations** in force for each stage — the minimum duration and the completion
window — are transmitted to the backend with the test record. They are fixed in the build and
are not needed to interpret the result; they are recorded **for traceability**, so a test can
be shown afterwards to have run under the timing it was supposed to. The start event and the
expiry behaviour are not recorded: the first is implied by the stage's start time and the
second is a rule rather than a value.
...
**S05.06** **The clock and its display start at different moments, and that is deliberate.**
The clock starts at the confirming action, silently, with nothing on screen. The countdown
appears a little later, timed to the narration that tells the user a wait has begun, so that
it arrives with the sentence that explains it rather than ahead of it. The elapsed time is
measured from the action throughout — a user who sees the countdown at 4:52 has already spent
those eight seconds, and no part of the wait is added back by the display being late.

**S05.44** The completion window is not counted down during the waiting phase and is not
shown, because nothing the user can do is yet at risk.

**S05.07** **The wait is not empty, and the flow runs through it.** The chat carries steps
during the waiting phase and the user works through them exactly as they work through any
other step of the flow. **They are not optional to the user** — there is no way round them and
nothing to decline. What separates them from the rest is only that **the clock may drop them**
if the user is slow. This document calls them **droppable**, never optional.

**S05.41** Each stage has a small number of steps the clock may **never** drop, because the
assay cannot complete without them. In the incubation stage they are shaking and opening the
Transfer Valve; in the colour-evolution stage they are shaking and scanning. The section that
carries a protected step is protected with it. Everything else in the stage is droppable.
**The distinction is about the assay, not about the user** — a droppable section is as
compulsory to the person holding the phone as a protected one, when they reach it.

**S05.40** **Droppable sections are elastic; the end-of-stage sequence is not.** The closing
sequence — transition bubble, zero, the completion the user witnesses, the instruction, the
control — is anchored to the clock and always happens on time. Droppable sections give way to
it.

**S05.49** **A section is the unit that gets dropped, and it is dropped whole.** A section is
a run of bubbles ending in a button and the user's action bubble. The flow decides before a
section begins whether enough of the stage remains for it, and if not the section never
starts. **A section already under way is never cut short** — once the user has seen its first
bubble it runs to its end. Dropping therefore happens only ahead of the user, never around
them, and **there is nothing for them to see when it happens**: a dropped section simply never
appears, so nothing has to be designed for it.
...
**S05.09** The card carries the countdown too, and **the same clock may be rendered in more
than one place at once** — the card's own and the persistent one below it. They are renderings
of a single computation from a single retained start time, so they cannot disagree; what is
forbidden is a second clock, not a second rendering.
...
affect neither the conduct nor the validity of the test. **It advances at a fixed interval** and repeats
where the wait outlasts the content; nothing spreads it to fit the time remaining.
...
**S05.12** During the incubation stage only, scan practice is a step of the flow like any
other, and a droppable one. It is **not** bounded by the stage that hosts it: FR-IMG-022 ends
it on a limit of its own. **That limit is not settled.** Milestone 1 runs on a provisional
30 seconds, which is what a demonstration needs, and later milestones are expected to want
nearer two minutes; the timing-flex study sets it with the rest. See SPEC-04, F04.7.
...
**S05.13** The transition is staged, not instantaneous. **The card yields before the
countdown reaches zero**, so that the chat can post a bubble saying the next step is about to
arrive. The countdown itself keeps running, relocated to the persistent element below.

**S05.45** **How early the card yields is derived, not chosen.** The interval is the length of
the **recorded audio** that has to play before zero — the transition bubble and anything with
it — so that it finishes as the countdown reaches zero rather than before or after it. Each
stage has its own interval because each has its own closing audio.

**S05.46** **The clock is the authority and the script is written to it.** The closing content
is timed from its own audio so that it lands on zero. It is not expected to run short or long,
and no delay is inserted to make it fit. Where the two do drift apart the countdown governs:
the flow proceeds at zero, not when the script happens to end.

**S05.47** Reaching zero is an event the user is meant to witness, not a disappearance. A
chime sounds, the countdown changes state to a completed one — its own presentation, its own
caption — holds there briefly, and only then retires. The instruction for the action follows
**after** that, not during it.
...
**S05.43** The **chime** sounds even where the application is not in the foreground, so that a
user who has put the phone down hears the stage end. **This is the chime alone.** The flow does
not otherwise continue in the background — nothing is narrated and no step advances; S05.03
governs the rest. Milestone 3.

**S05.15** The countdown does not change meaning at this moment, because **neither completion
window is shown to the user.** The countdown counts down to permission and then stops. What
replaces it is the instruction to act, not a second clock counting against them. Both windows
are enforced silently, and the two differ in length. **Neither length is settled**: milestone 1
runs on a provisional 3 minutes each so that a demonstration has a number, and the timing-flex
study sets the real ones.

**S05.16** The instruction for the action carries the same weight in both stages, but the
actions differ: incubation ends with a physical action on the device (the coloured release
button) confirmed in the chat; colour evolution ends by entering the scan.

**S05.48** **The control that confirms the action appears only once the instruction for it has
finished playing** — after the explanation and any demonstration, never alongside them. A user
cannot confirm an action they have not yet been told how to perform. Activating it posts the
confirmation and, in the same moment, starts the next stage's clock per S05.06.
...
inside the wait, and the wait is not extended to accommodate it. **The instruction therefore
consumes part of the completion window, and that is accepted**: the window is sized knowing it
opens with an instruction still playing.
...
*Traces: FR-TIM-007, FR-TIM-005, FR-FLW-008, FR-TIM-015 (M3).*
...
reason.** Invalidations that are not about timing — an unreadable code, a spent kit, an expired
one — are other documents' and are not enumerated here.
...
*Traces: FR-TIM-009, FR-TIM-011, FR-TIM-012 — **all three at milestone 2**. Invalidation on
timing grounds and the explanation of it arrive together, which is the point: a demonstration
does not void a test, and a build that voided one without saying why would be a poor
demonstration. FR-RDY-013, the rule about which resolution route is offered, and FR-KIT-007,
the twenty-four hour gate, are both milestone 3.*
...
**S05.36** Each stage raises **two** notifications while the application is in the background:
one when the waiting period ends, to bring back a user who put the phone down, and one before
the completion window closes, to warn that time is running out. They do different jobs at
different moments. FR-TIM-014 requires notification at the timing events that need the user
and deliberately does not count them, so which events and how many is settled here and can
change here without changing the requirement.

**S05.50** Where the user has refused notification permission, an SMS carries the same alert.
Milestone 5, and it does not weaken the tolerance of refusal in FR-COM-012 — the on-screen
countdown still carries the obligation, and this adds a route rather than a condition.
...
*Traces: FR-TIM-014, FR-COM-009, FR-COM-012, FR-COM-014 (M5).*
...
```
  ...
  |   whole sections dropped ahead of the user  |
  |   chat steps run; the clock may drop them   |
  |   card opens once earlier steps are done    |
  |   scan practice, a step (incubation only)   |
  +---------------------------------------------+
        |
        |  before zero: card yields,
        |  transition bubble plays,
        |  countdown moves to foot of screen
        v
  +---------------------------------------------+
  | HANDOVER                                    |
  |   nothing left to drop; the closing audio   |
  |   runs out exactly as the clock hits zero   |
  +---------------------------------------------+
        |
...
|---|---|---|---|
| WAITING | the step that starts the stage is confirmed | work through the chat steps, read the card, practise scanning (incubation only), leave the app | the handover begins, before the minimum duration elapses |
| HANDOVER | the countdown nears zero | hear the transition bubble | the countdown reaches zero |
...
the step they had reached, not at the step the clock implies — unless the window closed while
they were away, in which case the test is invalid and no step is resumed at all.
...
## 7. What carries over from Minuteful Kidney, and what does not

**This section sizes the design work. It does not delegate any of it.** Every behaviour in this
epic is specified in section 3, including the rows marked *unchanged* — that word means the
**mechanism** already exists and does not have to be invented, never that the behaviour is
defined elsewhere. It does not say how today's product works, and it is not a departures table.

The single change that drives most of the rest is the length of the wait: seventy-five seconds
in Minuteful Kidney, minutes here. Nearly everything below follows from that.
...
Two, and neither blocks building from this document.

| Ref | Question | Owner |
|---|---|---|
| FR-TIM-001 | **The timing values — all of them.** Milestone 1 runs on provisional numbers so that it can run at all: 5 minutes for incubation and 4 for colour evolution, 3 minutes for each completion window, 30 seconds for the scan-practice limit. **None is settled.** The timing-flex study sets them before milestone 3, Q-05 holds them in the register until it does, and this document is not where they become official. | Timing-flex study |
| FR-TIM-001 | **A suspected defect in the predecessor, to confirm.** A static reading of one platform suggests that after an analysis fails for a non-quality reason, the next test in the same session may run with no expiry deadline scheduled — the previous clock is not cleared, so a fresh start skips scheduling. Unconfirmed at runtime. It matters because the deadline is an assay control. **S05.02 and S05.38 already prevent it here**, because elapsed time is recomputed from a retained start time rather than held in a live timer, and a design that recomputes cannot leak a stale one. Worth knowing whether it is live in the shipping product. | Development team |

**Five `[UI placeholder]` blocks** sit in sections 3 and 4 — the countdown, the card, the
transition, the sad flow and the notifications. They are **not open items**: nothing about the
behaviour is undecided. They mark where this document implies an interface that has still to be
designed.

---

*Generated against QACR-APP-FR-01 Rev 1.27 and QACR-APP-EPIC-01 Rev 1.21. Every
