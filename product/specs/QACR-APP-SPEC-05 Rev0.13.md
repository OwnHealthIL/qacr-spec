...
| Revision | 0.13, draft for review — Rev 0.12 review applied; not yet re-reviewed |
...
**Guy's review of Rev 0.12 is applied at this revision**, and its largest correction is a word.
The document called the steps that run through a wait *optional*. They are not: the user goes
through them like any other step and has no way round them. What separates them is only that
**the clock may drop them if the user is slow**. They are called **droppable** throughout now,
and the steps the clock may never drop — shaking, the Transfer Valve, scanning — are protected
because the **assay** needs them, not because the user is obliged to them.

**That also dissolves what Rev 0.12 raised as a contradiction.** The practice scan is a step
the user passes through, which is what SPEC-04 and FR-IMG-022 require, *and* one the clock may
drop, which is what the mockup shows. Mandatory-to-the-user and mandatory-to-the-assay are
different things, and only the second is what "droppable" is about. The open item is withdrawn.

**Three numbers arrived**: the scan-practice limit is 30 seconds at milestone 1, the completion
windows are 3 minutes each at milestone 1, and the card's tips advance at a fixed interval
rather than being spread to fit.

**One statement was wrong in the other direction.** Background audio was written as the
application continuing to run like a media player; it is the **chime alone** that has to reach
a user who put the phone down. Nothing else continues in the background.
...
recomputed. The steps behave as any other chat step does; **the clock sits above them and runs
regardless.** The user resumes where they stopped and continues from there, and where too
little of the stage is left the droppable steps are dropped per S05.49. They may return into a
phase they never saw begin. Where the window closed while they were away, S05.04 applies
instead.
...
Transfer Valve; in the colour-evolution stage they are shaking and scanning. Everything else in
the stage is droppable. **The distinction is about the assay, not about the user** — a
droppable step is as compulsory to the person holding the phone as a protected one.

**S05.40** **Droppable steps are elastic; the end-of-stage sequence is not.** The closing
sequence — transition bubble, zero, the completion the user witnesses, the instruction, the
control — is anchored to the clock and always happens on time. Droppable steps give way to it.

**S05.49** The dropping happens **before or during** a droppable block, not at the end of the
stage. Before a block begins the flow checks whether enough of the stage remains for it, and
does not start it if not; a block already running is cut short when the time it would need is
gone. By the time the closing sequence is due there is nothing left to skip. Where the user
has kept pace and the time is there, nothing is dropped and they simply continue through the
flow as usual.
...
```
  ...
  |   droppable steps dropped here, if at all   |
