...
| Revision | 0.9, draft for review — current to FR Rev 1.25; not yet reviewed |
...
**FR-TIM-006 changed again at FR Rev 1.25**, and it answers a question this document had
left open: an exceeded sample-freshness limit **invalidates the test** rather than blocking the
step. S05.27 and the open item that asked it are updated.

**Four requirements this document traces to moved at FR Rev 1.24**, in the SPEC-04
requirements pass. **FR-TIM-008** now admits an exemption for demonstration mode, which
produces no result for a patient — so a demonstration may cross a minimum duration, and
nothing in this document's timing statements applies to a demonstration run.
**FR-IMG-022** makes the practice scan a step of the flow the user passes through rather
than one they may start, ended by a limit of its own rather than by the end of the wait —
**section 3.3 and S05.32 need re-reading against that at review, because this document was
written when the practice scan ended with its wait.** **FR-FLW-008** gains the silence
scope and the stop-on-confirm rule, and **FR-KIT-007** the show-until-confirmed rule;
neither bears on a timed wait. No statement in this document has been rewritten — this
revision refreshes the citation and records what moved, and the reconciliation is review
work.
...
framing when it expires has an expired test. Whether the scan in progress is allowed
to finish is an open item — see section 9.
...
**S05.25** After an invalidated test the user returns to the home screen, and the
twenty-four hour new-kit gate applies to their next attempt as it does after a
completed test.

**S05.35** The current product does not return the user home. It tells them the kit is
spent and then takes them through a guided exit that ends at customer support. So the
recreation baseline is a guided exit, not a dismissal. Whether QACR keeps that or ends at the home screen per S05.25 is an open item,
and it matters more here than in Minuteful: a QACR user has lost ten minutes and a kit,
and the difference between being handed to support and being returned to a home screen is
the difference between a resolved and an unresolved support case.
...
**S05.27** If the limit is exceeded, the test is invalidated. The user is told when they
attempt to connect the Sample Pod, and told that a new kit is needed — not merely held at
the step. **This closes what was an open item in Rev 0.8.**
...
| Invalidation on timing expiry | Mechanism exists. Additions: two timing failures rather than one, and the distinction between them. **The route out differs** — today a guided exit to support, not a return to the home screen. S05.35. |
| Sample-freshness window | Unchanged mechanism. **Changed outcome:** expiry invalidates the test rather than blocking the step, and the user is told a new kit is needed rather than being routed to support. The limit also differs and remains open. S05.27. |
...
| S05.19 | If the scanning window expires while a capture is in progress, is that capture allowed to finish? | New, raise at review |
...
| S05.35 | On timing expiry, does the user get Minuteful's guided exit to support, or the home screen? | New, raise at review |
...
| S05.37 | **Is a safety margin wanted on the reaction, and is it part of the minimum duration?** The predecessor keeps one by displaying more time than it enforces. If QACR wants a margin it should be in the duration itself, where the timing-flex study sets it and verification tests it. Needs deciding before the study fixes the values. | New, raise at review |
| S05.27 | ~~Does the expiring sample route the user to support, or tell them a new kit is needed?~~ **Answered at the FR Rev 1.25 review: it invalidates the test and the user is told a new kit is needed.** FR-TIM-006 amended accordingly. | Closed |

---

*Generated against QACR-APP-FR-01 Rev 1.23 and QACR-APP-EPIC-01 Rev 1.17. Every
