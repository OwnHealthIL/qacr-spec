...
| Revision | 0.14, draft for review — Rev 0.13 review applied; not yet re-reviewed |
...
**Guy's review of Rev 0.13 is applied at this revision.** Two comments, both narrowing what
dropping means.

**A section is the unit, and it is dropped whole.** A section runs from its first bubble to a
button and the user's action bubble. The flow decides *before* a section starts whether the
stage has room for it; a section already under way is never cut short. The previous revision
had one being truncated mid-way, and that cannot happen.

**Which is why the user always meets the protected step.** The previous revision said they
might return into a phase they never saw begin. They cannot: protected steps are never dropped
and nothing is skipped past the user, so however long they were away, the instruction for the
action is something they see.

*Earlier revisions are in git. This note carries only the change since the last one Guy read,
and it comes out when the document goes ready.*
...
| FR-CFG-003 | ~~Whether the timing values in force are recorded with the test.~~ **Answered: the durations are** — S05.39 sends the minimum duration and the completion window with the exam, for traceability rather than interpretation. What remains is only whether FR-CFG-003's wording covers them or needs widening. | FR doc, raise at review |
| S05.36 | **The notification set is an amendment to FR-TIM-014, not a new requirement.** Two per stage: one when the waiting period ends, one a minute before the completion window closes. FR-TIM-014 covers only the second and only as a warning; widening it covers both. The behaviour statements here follow whatever wording it lands on. | Amendment |
| S05.36 | The study's SMS fallback where notification permission was refused. **Wanted as a milestone-5 requirement** rather than dropped — it is a real re-engagement route and belongs in the record as future development. FR-COM-012 currently states the opposite intent and SMS is a backend capability adjacent to FR-COM-013. | New requirement, M5 |
...
| S05.43 | **The chime sounding while the application is in the background has no requirement.** It is the chime alone — not the narration, and not the flow continuing — so what is needed is narrow: permission for the application to emit a sound it did not schedule in the foreground. iOS and Android differ, and it may need a platform capability declared at release. | New requirement |
