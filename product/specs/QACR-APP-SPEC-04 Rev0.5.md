...
| Revision | 0.5 — draft; Rev 0.4 review answers and the Phase 1 sweep applied |
...
**What changed from Rev 0.4.** The sweep that Phase 1 of the spec procedure calls for had never
been run for the four features the developers' behaviour export does not cover. It has now, from
the team's extracted evidence. It produced two departures — how demonstration mode is determined,
and how long a silenced flow stays silent — and one finding against the requirements that is
larger than this document: **three milestone-1 requirements assume a backend-supplied
demonstration state while the requirement that retrieves configuration sits at milestone 3.**
See section 4, U5.
...
stage before the minimum time has elapsed, and admits no exemption. Both demonstration controls
cross it. There is no clinical conflict — a demonstration produces no result for a real
patient, so no assay timing is being defeated — but the requirement as written does not say so,
and that is a requirement to amend rather than a rule to bend in a spec. See section 5.1.
...
a practice scan and requires it to *end when the waiting period ends*. **The amendment is
approved and has not yet landed** — until it does, this document and the requirement it traces
to disagree. That is what section 5.1 is for, and it is why this document is not yet ready.

---

## 4. Still undecided

**Three of these four are questions the development team has to answer before Guy can.** They
are facts about the current product, not decisions, and putting them to Guy first is what has
stalled each of them at least once. Only U1 is a decision waiting on nothing.
...
| **U3** | Whether retained test state must survive an application update. **It depends on what an update actually erases on the platform that erases anything**, which is the developers' to establish; the decision is Guy's after that. | Developers, then Guy |
| **U5** | **Which requirement delivers configuration at milestone 2, and does the demonstration assumption at milestone 1 need correcting?** FR-CFG-001, which retrieves the configuration set from the backend, is milestone 3. Three milestone-1 requirements already assume a backend-supplied demonstration state — FR-FLW-007, FR-RES-006 and FR-CFG-004 — so the corpus has a milestone-1 obligation resting on a milestone-3 capability. D11 resolves it for this feature by determining the mode in the build at milestone 1; the other two requirements belong to SPEC-09 and SPEC-01 and carry the same assumption. | Guy |
| **U4** | What the two current behaviours actually amount to when a parameter governing the inclusion or omission of a section is not supplied — **what an alternative section is, and what a user meets when a section drops entirely.** The developers' to establish first; the decision is Guy's after that. Behaviour is as the current product's and no requirement changes, but *same as today* does not pick between two different things, and one of them is how a section that is required to exist stops being presented. | Developers, then Guy |
...
|---|---|---|---|
| FR-STA-003 | Define the point at which a test becomes active — the user activating the start-test control — and state the **UI reset** that applies outside it. The prohibition on an inactivity timeout during a test says nothing about when a test begins, and a UI reset does apply before that point. | 1 | D4, U2 |
| FR-FLW-004 | Completed instructions remain available to the user while the flow continues. The flow follows new content as it arrives. | 1 | D3 |
...
| FR-FLW-007 | The demonstration controls may advance past anything the chat presents, including timing windows and confirmation steps. Its note must also stop asserting that demonstration mode is never a build-time state, which D11 makes untrue at milestone 1. *Narrowed at the sweep: the mode being evident is already a recreation — see section 6.2.* | 1 | D9, D11 |
| **FR-TIM-008** | Admit the demonstration exemption the requirement currently does not. **Approved at the Rev 0.4 review.** | 1 | D9 |
| FR-FLW-008 | A silenced flow stays silent for the current test session and no longer. The requirement provides the control and says nothing about how long the choice lasts. | 1 | D12 |
| FR-IMG-022 | A practice scan is a step of the flow the user passes through rather than one they may start; and the limit that ends it is its own rather than the wait's. **Approved at the Rev 0.4 review.** | 1 | S04.01, S04.04, U1 |
...
| FR-STA-007 | Draw the line the corpus leaves open between FR-STA-007's *invalidate the incomplete test* and FR-RDY-012's *retransmit what did not arrive*. A test whose capture succeeded and awaits only transmission is **not** an incomplete test: it and its images are retransmitted, not invalidated. A test whose capture failed **is** invalidated. | 1 | Review answer |

**On FR-STA-007.** Guy's answer was that the requirement *can say so or not* — the rule itself
is what has to be unambiguous. It is proposed anyway, because the ambiguity it closes is
whether the software invalidates a test that is about to produce a valid result, and that is
not a thing to leave to inference in a Class B submission. **The rule is settled either way**
and is stated in section 6.2, so nothing waits on the FR review.
...
Both of these rest on behaviour that was marked `correct` at review — meaning the current
behaviour is what QACR should do — while each describes a requirement the product does not
carry. Under the
review's own rule they should be dropped. They are recorded rather than dropped because for a
Class B submission an unstated capability is a gap rather than a preference. **This is a
decision, not a defect to fix here.**
...
| FR-STA-003 | Retained state survives an application update, and where it cannot be read back the user is told a test could not be recovered rather than shown the start-up state. **Held pending U3.** | 1 |
...
| FR-STA-007 — an abandoned test's data is discarded, the invalidation reported, and a later result not presented | Rests on a misreading of the case. A capture that finished and only awaits upload is not an abandoned test: it resumes transmitting at the next start and reaches a result, as today. **The transmission mechanism is not to be touched.** What remains is a wording question, in section 5.2. |
| FR-STA-007 — something is written down by which an incomplete test can be identified on reopening | Hangs on the application's ability to track test state, individually and against the backend order status. The requirements for that are unchanged, so this needs no requirement of its own. |
| FR-FLW-006 — a parameter governing a section has a value applied when it is not supplied | Behaviour is as the current product's and no requirement changes. **U4 records what that leaves unanswered.** |
...
| fast-forwarding past a wait and its audio | In scope, for demonstration partners only, and as **two** controls rather than one. They may skip all timing windows; no real result is produced, so assay timing is unaffected. | **D9**, and the FR-FLW-007 proposal |
| who owns the QACR section list | The `urine-bible` content repository, along with the whole structure, flow logic and content of the chat flow. | Closed. *Not in this brief* |
| whether an abandoned test's images can reach analysis | **The case was misread, and the line is now drawn.** A capture that succeeded and awaits only transmission is not an abandoned test: it and its images wait to be sent, the software keeps trying across a kill, and the test reaches a result — as today, and the mechanism is not to be touched. Invalidation is what happens when the capture *failed*. | Closed. Stated in 6.2; proposed as an amendment in 5.1 |
...
| Question | Answer |
|---|---|
| Do the parameters that decide what the flow opens with carry over? | **Yes, as today.** U4 asks only which of two current behaviours applies when one is missing. |
...
| What happens when retained state cannot be read back? | **As today.** U3 asks only about an application update. |
...
|---|---|---|
| F04.1 Step sequencer and confirmation model | FR-FLW-001, FR-FLW-004, FR-FLW-005, FR-FLW-006 (M3) | as-is except **D1, D2, D3, D5** · U4 open |
| F04.2 Instruction media, replay and spoken delivery | FR-FLW-002, FR-FLW-003, FR-FLW-008 | as-is except **D3, D10** |
| F04.3 State retention and interruption tolerance | FR-STA-002, FR-STA-003, FR-STA-005 | as-is except **D4, D6, D7** · U2, U3 open |
