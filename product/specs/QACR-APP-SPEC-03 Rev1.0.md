...
| # | Feature | What changes | Milestone | Driven by |
| --- | --- | --- | --- | --- |
| **D1** | F03.1 | **The user may present the kit identifier for reading.** Where the software cannot read it automatically, it asks the user to present the code, and invalidates the test only once that has also failed. Today the automatic reads are all there is. Whether a missing identifier refuses the test at all stays configuration-determined, as it is today. | **5** | FR-KIT-009 |
| **D2** | F03.1 | **One kit-identifier template, applied identically on both platforms.** Today each platform enforces a different rule for what counts as well-formed, so a code one accepts the other rejects. The template itself is a value, not a departure — see U1. | 3 | FR-KIT-003, FR-PLT-006 |
| **D3** | F03.1 | **A kit that is not this product's presents the general failure.** Today one platform names the condition and the other presents an empty message. QACR states nothing specific about the kit, because nothing specific has been established about it. | 3 | FR-KIT-008 |
| **D4** | F03.1 | **A refusal the software cannot attribute presents the general failure.** Today an unrecognised kit refusal is reported as an already-used kit, which may be untrue and sends the user to the wrong troubleshooting route. The software never asserts a reason it has not been given. | 3 | FR-KIT-008 |
| **D5** | F03.2 | **An expired kit routes to troubleshooting**, as the other kit failures do. Today it is the one kit failure with no guided conversation. | **5** | FR-KIT-005 |
| **D6** | F03.2 | **Confirming the new-kit notice starts the test.** Today the notice returns the user to the lobby and they must start the test again with a second tap that asks nothing — so nothing is confirmed at the moment the flow begins, which is what the requirement asks for. | 3 | FR-KIT-007 |
| **D7** | F03.2 | **Declining a new test dismisses the warning and does nothing else.** One platform opens a results history screen instead; that screen is future development for QACR, so there is nowhere for it to go. Recorded as a departure rather than left implicit, so nobody recreates the route by default. | 3 | FR-PRT-015 (M5) |
| **D8** | F03.2 | **Where the user has no unused kit left, the message comes first, and support is the only route offered unless the backend reports more than one patient.** Today one platform opens a switch-user screen with no message first. Switching user is offered only where the backend reports more than one patient against the phone number or the address, and that case is future development for QACR, so support is the route that applies. | 3 | FR-RDY-014, FR-AUT-020 (M5) |
| **D9** | F03.2 | **An expired kit does not count as a completed test** for the twenty-four-hour gate. The user has been told the kit is expired and will not reasonably try it again; if they do, they are refused again on the same grounds. Today one platform records it as a completed test and warns on the next attempt, and the other does not. | **5** | FR-KIT-007, FR-KIT-005 |
| **D10** | F03.2 | **The twenty-four-hour window excludes its endpoint**, identically on both platforms. Today they disagree at exactly twenty-four hours: one warns, one does not. Now stated in FR-KIT-007. | 3 | FR-KIT-007, FR-LCM-006 |
...
| Does the recency warning need re-acknowledging on a second attempt in the same window? | **No.** One acknowledgement stands until the next test completes, as today. |
