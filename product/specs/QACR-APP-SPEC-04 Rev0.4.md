...
| Revision | 0.4 — draft; Rev 0.3 review answers applied, not yet re-reviewed |
...
**What changed from Rev 0.3.** Both of the previous revision's open judgement calls were
answered. The line between a capture that succeeded and one that failed is now stated plainly
rather than raised as a question, because it is the one thing here a developer must not get
wrong. U4 joins U2 and U3 in needing a fact from the development team before it can be put to
Guy — three of the four now do, which is itself worth noticing.
