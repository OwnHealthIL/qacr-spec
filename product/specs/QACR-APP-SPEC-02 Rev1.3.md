...
|---|---|
| F02.1 Phone number entry and lookup | **recreated** — Minuteful Kidney, except departure D1 |
| F02.2 One-time password verification | **recreated** — Minuteful Kidney |
| F02.3 Invite code for unregistered numbers | **recreated** — Minuteful Kidney |
| F02.4 Date-of-birth confirmation at test start | **recreated** — Minuteful Kidney, except departure D2 |
| F02.5 Session and token lifecycle | **recreated** — Minuteful Kidney |
| F02.6 Authentication guidance and failure support | **recreated** — Minuteful Kidney |
| F02.7 Consent acknowledgement | **recreated** — Minuteful Kidney |
| F02.8 First-run onboarding | **recreated** — Minuteful Kidney |
...
**Out of scope.** Readiness and eligibility, which is SPEC-01 — including the refusal the  
backend returns where the user is not authorised to test, which SPEC-01 covers as one of the  
refusals the application must present. Results access and its PIN, which is SPEC-13.
...
| --- | --- | --- | --- | --- |
| **D1** | F02.1 | A backend that does not answer the registration lookup and a number that is not registered produce the same message, which tells the user something about their number that has not been established. | A non-response is not reported to the user as an unregistered number. | FR-AUT-003 |
| **D2** | F02.4 | Configuration has two states for the date-of-birth step: enforce the check, or skip the validation. The step itself is always present in the chat. | A third state, which suppresses the step from the chat entirely. Intended for demonstration partners. | FR-AUT-012, unchanged — see Q-101 |
