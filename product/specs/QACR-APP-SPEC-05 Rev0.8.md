...
| Revision | 0.8, draft for review — current to FR Rev 1.24; not yet reviewed |
...
**S05.27** If the limit is exceeded, the user is prevented from proceeding when they
attempt to connect the Sample Pod, and told that the sample is no longer usable.
...
what S05.26 and S05.27 describe. Two things to settle deliberately rather than by
inheritance. The limit differs — the predecessor's is shorter, and the value here is
Guy's to set. And on expiry the current product routes the user to support rather than
telling them a new kit is needed; see section 9.*
...
| Sample-freshness window | Unchanged mechanism. The limit differs, and today expiry routes to support; both are open. S05.27. |
...
| S05.27 | Does the expiring sample route the user to support, as it does today, or tell them a new kit is needed? | New, raise at review |
