...
| Revision | 0.3 — draft; Rev 0.2 review answers applied, not yet re-reviewed |
...
**What changed from Rev 0.2.** D3 was wrong and is corrected: the view follows new content
while the flow plays, with no exception for a user reading something earlier. D9 splits into
two controls. Three proposed requirements were declined and one raised in their place. One
undecided closed, one opened, and two changed owner to the development team, who are asked for
the facts before the question can be put.
...
this brief can grant itself; both are in section 5.1 as amendments.

---

## 4. Still undecided

Two of these are questions the development team has to answer before Guy can. They are facts
about the current product, not decisions, and putting them to Guy first is what has stalled
them twice.
...
| **U4** | Which behaviour applies when a parameter governing the inclusion or omission of a section is not supplied. The instruction is that it works as the current product does and no requirement changes — but **the current product does two different things**: one platform falls back to the alternative section, the other drops the section entirely and reports nothing. *Same as today* does not pick between them, and one of them is how a section that is required to exist stops being presented. | Guy |
...
| FR-FLW-007 | Demonstration mode is evident within the flow, and the demonstration controls may advance past anything the chat presents, including timing windows and confirmation steps. **This is the exemption FR-TIM-008 does not currently admit**, and it is safe because a demonstration produces no result for a real patient. | 1 | D9 |
...
These rest on behaviour that was marked `correct` at review — meaning the current behaviour is
what QACR should do — while each describes a requirement the product does not carry. Under the
...
| FR-STA-007 | A test whose capture and analysis completed is not an incomplete test, however the application ended afterwards. **This is the line the review said the corpus does not draw**, and the review is right that it is not drawn: FR-STA-007 requires an incomplete test to be invalidated and FR-RDY-012 requires unsent data to be retransmitted, and nothing says which applies to a finished capture that never uploaded. Guy has now drawn the line in product terms — see section 6.1 — and this asks only whether the requirement should say so. **It changes no mechanism.** | 1 |
...
| whether an abandoned test's images can reach analysis | **The case was misread.** A capture that finished its scanning and analysis and only awaits upload is not an abandoned test: it resumes transmitting at the next start and reaches a result, as today, and that mechanism is not to be touched. The invalidation obligation was never about it. | Closed. Proposal declined; wording question in 5.2 |
...
| Does a finished capture that never uploaded still reach a result? | **Yes, as today.** It resumes at the next start. The mechanism is not to be touched. |
