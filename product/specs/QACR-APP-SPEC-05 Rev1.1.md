...
| Milestones | 1, except telling the user why a test was invalidated at 2, the sample-freshness window and the background end-of-stage signal at 3, the expiry notifications at 4, and the SMS alternative at 5 |
| Domains | iOS, Android |
| Traces to | QACR-APP-FR-01 Rev 1.26 · QACR-APP-EPIC-01 Rev 1.20 |
...
it on a limit of its own. **That limit is 30 seconds at milestone 1**, which is what a
demonstration needs; later milestones may extend it to around two minutes. See SPEC-04, F04.7.
...
are enforced silently, and the two differ in length — TBD from the timing-flex study, with
**3 minutes each at milestone 1** so a demonstration has a number to run on.
...
*Traces: FR-TIM-009, FR-TIM-011. **FR-TIM-012, which requires the user to be told the reason
and that a new kit is needed, is milestone 2** — so at milestone 1 the software invalidates on
timing grounds with no requirement to say why. S05.22 describes the milestone-2 behaviour.
FR-RDY-013, the rule about which resolution route is offered, and FR-KIT-007, the twenty-four
hour gate, are both milestone 3.*
...
| Ref | Question | Owner |
|---|---|---|
| FR-TIM-001 | **The timing values.** Minimum durations are set — 5 minutes for incubation, 4 for colour evolution. The **completion windows** are not: they run on 3 minutes each at milestone 1 so a demonstration has a number, and the timing-flex study sets them before milestone 3. Card thresholds and the scan-practice limit beyond milestone 1 sit with them. | Timing-flex study |
...
*Generated against QACR-APP-FR-01 Rev 1.26 and QACR-APP-EPIC-01 Rev 1.20. Every
