...
**Eight features, all recreated.** Authority is per feature — see below.

| | |
|---|---|
| Document | QACR-APP-SPEC-01 |
| Revision | 1.6 — **ready**. Reviewed and settled; build from it. |
...
| Milestones | 3, except the camera check at 1, the configuration set and server-side demonstration enforcement at 2, exclusive store distribution at 4, and two configuration items at 5 as future development |
...
|---|---|
| F01.1 Store distribution and install-time compatibility | **recreated** — Minuteful Kidney |
| F01.2 Supported-device policy and run-time eligibility | **recreated** — Minuteful Kidney |
| F01.3 Device integrity check | **recreated** — Minuteful Kidney |
| F01.4 Pre-test resource and permission checks | **recreated** — Minuteful Kidney, except departure D1 |
| F01.5 Blocked-state pattern | **recreated** — Minuteful Kidney |
| F01.6 Deferred-upload recovery at start-up | **recreated** — Minuteful Kidney |
| F01.8 Device and version telemetry on the test record | **recreated** — Minuteful Kidney |
| F01.9 Run-time configuration | **recreated** — Minuteful Kidney, except departure D2 |

**Nothing here is new**, so the precedence rule below applies to every feature without
exception. This is the one document in the set where that is true of all of it.
...
**The conditions themselves are Appendix I of the requirements**, which records every
condition that refuses a test — where each is established, and whether it blocks, informs
or is switched off for that partner. This brief does not repeat them and must not be read
as naming any of them. FR-RDY-011, FR-RDY-014 and FR-CFG-006 state the rule; Appendix I
carries the cases.
...
|---|---|---|---|---|
| **D1** | F01.4 | One platform exempts a charging device from the battery check and the other does not. | The battery check is the level and nothing else. A device below the minimum is blocked whether or not it is charging, because a cable can be pulled out during a ten-minute wait. | FR-RDY-007 |
| **D2** | F01.9 | The configuration set a test ran under is not recorded, and does not travel with the test data. | The configuration set that was in force travels with the test data to the backend, so a completed test carries the configuration it ran under. | FR-CFG-003 |
...
| F01.9 Run-time configuration | FR-CFG-001 (M2), FR-CFG-002 (M2), FR-CFG-004 (M2), FR-CFG-003, FR-CFG-006 · future development at M5: FR-CFG-008, FR-LCM-018 | as-is except **D2** |

**Three configuration requirements moved at FR Rev 1.24**, on the SPEC-04 review's finding
that milestone 1 carried three requirements assuming a backend-supplied demonstration state
while the requirement retrieving configuration sat at milestone 3. Retrieval and schema
validation (FR-CFG-001, FR-CFG-002) move to **milestone 2**, where the configuration set
stops being demonstration-only and carries a real test partner; server-side enforcement of
demonstration behaviour (FR-CFG-004) moves to **milestone 2** with them, because it cannot
precede the channel that delivers it. Milestone 1 may determine demonstration mode in the
build. **Nothing in this brief changes** — no departure, no undecided, no disposition.
