...
**Seven features: six recreated, one new.** Authority is per feature — see below.
...
|---|---|
| F04.1 Step sequencer and confirmation model | **recreated** — Minuteful Kidney. This document records only departures D1, D2, D3, D5 |
| F04.2 Instruction media, replay and spoken delivery | **recreated** — Minuteful Kidney, except departures D3 and D10 |
| F04.3 State retention and interruption tolerance | **recreated** — Minuteful Kidney, except departures D4, D6, D7 |
| F04.4 Termination detection and invalidation | **recreated** — Minuteful Kidney, except departure D8 |
| F04.5 Cancellation | **recreated** — Minuteful Kidney, except departures D4 and D5 |
| F04.6 Demonstration navigation | **recreated** — Minuteful Kidney, except departures D9 and D11 |
| F04.7 Scan practice | **new** — this document, section 3, **for the practice framing only**: that it is a step the user passes through, when it is offered, how it ends and what it does not produce. Minuteful Kidney has no practice scan, so there is nothing to recreate there. **The scanning interaction it reproduces is E06's and is largely unchanged from Minuteful Kidney** — this document neither defines it nor departs from it |

**F04.7 is the exception the precedence rule below does not cover, so read this first.** That
rule sends a disagreement to Minuteful Kidney, and for six of these features that is right.
For F04.7's *framing* it is not: nothing there answers when a practice scan is offered or what
it produces, and section 3 is the whole of that specification. **For section 3, this document
wins.**

**But the scan the user actually performs during practice is not new**, and this document does
not describe it. It is E06's, and it is largely a recreation of Minuteful Kidney's scanning
interaction — which is the point of practising: the user meets the real thing. A question about
framing comes here; a question about the scan itself goes to E06 and, through it, to Minuteful
Kidney.
...
> **For the six recreated features: where this brief and Minuteful Kidney disagree on anything
> not in the departures table, Minuteful Kidney is right.** Raise it rather than implementing
> the brief. **This does not apply to F04.7**, which has no equivalent to defer to.
...
| # | Feature | Today | In QACR | Driven by |
|---|---|---|---|---|
| **D1** | F04.1 | The procedure runs as a fixed set of named sections shipped with the application, ending in a dipping step. | The flow is built from scratch. Its sections, their order and their number are QACR's own and are not the predecessor's; there is no dipping step. The engine's rules are unchanged and apply to whatever sections QACR defines. | FR-FLW-001, FR-FLW-005 |
| **D2** | F04.1 | The camera-permission request follows the sentence explaining it, but the platforms time it differently: one waits for that sentence to finish being spoken, the other waits a fixed interval. | *Platform disagreement — iOS.* The request is presented once the explanation has been given, rather than after a fixed interval. | FR-RDY-009 |
| **D3** | F04.1, F04.2 | While the flow is playing, the platforms treat a user who has scrolled back through the transcript differently: one carries the view to each new message anyway, the other leaves the view where the user put it. | *Platform disagreement — iOS.* The view is carried to each new bubble as it is displayed. There is no exception for a user who has moved back through the transcript. The transcript itself remains available; it is the view that follows the flow. | FR-FLW-004 |
| **D4** | F04.5, F04.3 | The dipping step is the point at which the sample is committed. It is the step the exit warning is worded against, and the step after which an absence no longer resets the flow. | That point is the step confirming the Urine Collection Cup was used. Everything that hangs off it moves with it: what the exit warning says, and whether an absence triggers the UI reset. | FR-STA-008, FR-STA-003 |
| **D5** | F04.5, F04.1 | The exit control asks for confirmation with one message, worded for the state after the dip — that quitting will need a new kit — whichever step it was invoked from. | The exit confirmation is not one message. It presents a message appropriate to the step it was invoked from — before the cup was used, and after. | FR-STA-008 |
| **D6** | F04.3 | Transmission of a captured test behaves differently when the user leaves the application: on one platform it continues, on the other it stops as soon as the system stops the app. Neither tells the user anything about it. | *Platform disagreement — iOS.* Transmission continues after the user leaves the application. Nothing is shown to the user about it, then or on return. | FR-STA-005 |
| **D7** | F04.3 | The platforms part company once a timeout has been presented and dismissed: one stops enforcing the absence window for the rest of the run, the other starts enforcing it again from the moment the alert is dismissed. | *Platform disagreement — iOS.* The absence window applies again from that moment. It does not stop applying for the rest of the run. | FR-STA-003 |
| **D8** | F04.4 | A timeout recorded before the application died is lost with it. Reopening the test screen shows the chat with no message on it, so the user is left in a flow that has already ended and is never told why. | A recorded timeout survives the application dying. The user is shown the failure and continues into the troubleshooting flow. If the application was killed before the failure was presented, the user opens at the start-up state and is shown the failure there. | FR-STA-007, FR-TIM-012 |
| **D9** | F04.6 | The demonstration route is a single bypass. It jumps straight to the scan step and removes the countdown before it; nothing in between can be reached. | **Two controls**, not one: a navigator that moves between sections of the flow, and a fast-forward that runs to the end of the current section. Between them they may pass over any bubble, any audio and any timer the chat presents. | FR-FLW-007 |
| **D10** | F04.2 | An instruction video does not open full screen. | An instruction video opens full screen. This is already how the QACR usability application behaves. | FR-FLW-002 |
| **D11** | F04.6 | Demonstration mode is determined from the backend throughout. | **Determined in the build at milestone 1**, and by configuration retrieved before a test starts **from milestone 2**, where it will carry more than demonstration. The QACR application has no configuration channel yet. | FR-FLW-007 |
| **D12** | F04.2 | A silenced flow stays silent across tests; the choice is remembered. | A silenced flow stays silent **for the current test session** and no longer. | FR-FLW-008 |
